...
class MovieBase(BaseModel):
    ...
    rating: Optional[float] = Field(
        None, ge=0.0, le=10.0, description="Calificación (0–10)"
    )
    description: Optional[str] = Field(
        None, max_length=1000, description="Breve descripción"
    )
...
class MovieListResponse(BaseModel):
    ...
    data: List[dict] = Field(
        default_factory=list, description="Listado de películas (dict) en este paso"
    )
